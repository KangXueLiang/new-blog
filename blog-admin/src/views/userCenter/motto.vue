  ...
  <div class="container shop-management-container">
    <header>
      <el-form :inline="true" class="demo-form-inline" style="display: inline-block; vertical-align: middle">  
        <el-form-item>
          <el-button type="primary" @click="addMotto()">新增Motto</el-button>
        </el-form-item>
      </el-form>
    </header>
    <main>
...
export default {
  ...
  methods: {
    ...
    // 获取列表
    getList() {
      this.$http.get('api/mottoes').then(res => {
        this.list = res.data
      }).catch(e => {
    ...
    // 新增motto
    addMotto() {
      this.$prompt('请输入新的motto', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消'
      }).then(({ value }) => {
        this.$http.post('api/mottoes', {
          content: value
        }).then(() => {
    ...
    // 删除分类
    deleteItem(id) {
      this.$confirm('要确定删除此motto吗？', '删除分类', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
      }).then(() => {
        this.$http.delete(`api/mottoes/${id}`, {
          id
        }).then(() => {
