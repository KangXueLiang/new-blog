...
router.get('/articlesByTitle', async (req, res, next) => {
  try {
    ...
    responseClient(res, 200, 0, '返回成功', {
      posts: result
    })
  } catch (e) {
    responseClient(res, 200, -1, '返回失败', e.message)
  }
});
...
router.get('/articlesByTag', async (req, res, next) => {
  try {
    const tag = await Category.findOne({ name: req.query.tag })
    const oid = new MyObjectId(tag._id)
    // console.log('看一下tag', tag._id)
    const result = await Article.find({status: {$ne: false}, category: oid}).sort({_id: -1});
    res.header({
      status: true,
      time: new Date().getTime(),
    });
...
router.get('/articlesByPV', async (req, res, next) => {
  try {
    ...
    let result = await Article.find({status: {$ne: false}}).sort({pv_count: -1, _id: -1}).limit(7)
    responseClient(res, 200, 0, '返回成功', result)
  } catch (e) {
    responseClient(res, 200, -1, '返回失败', e.message)
  }
});
