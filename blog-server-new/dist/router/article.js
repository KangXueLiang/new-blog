...
router.get('/articlesByTitle', async (req, res, next) => {
  try {
    ...
    (0, _constant.responseClient)(res, 200, 0, '返回成功', {
      posts: result
    });
  } catch (e) {
    (0, _constant.responseClient)(res, 200, -1, '返回失败', e.message);
  }
});
...
router.get('/articlesByTag', async (req, res, next) => {
  try {
    const tag = await _index.Category.findOne({ name: req.query.tag });
    const oid = new MyObjectId(tag._id);
    // console.log('看一下tag', tag._id)
    const result = await _index.Article.find({ status: { $ne: false }, category: oid }).sort({ _id: -1 });
    res.header({
      status: true,
      time: new Date().getTime()
    });
...
router.get('/articlesByPV', async (req, res, next) => {
  try {
    ...
    let result = await _index.Article.find({ status: { $ne: false } }).sort({ pv_count: -1, _id: -1 }).limit(7);
    (0, _constant.responseClient)(res, 200, 0, '返回成功', result);
  } catch (e) {
    (0, _constant.responseClient)(res, 200, -1, '返回失败', e.message);
  }
});
